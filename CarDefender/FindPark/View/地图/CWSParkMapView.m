...
-(void)creatNormalView{
    //1.创建level视图
    _levelWidth = 38;
    _levelBtn = [Utils buttonWithFrame:CGRectMake(51, self.frame.size.height - 133-40, 38, 27) title:nil titleColor:KBlackMainColor font:kFontOfSize(8)];
    ...
    //2.创建回到默认点视图
    [self creatBtn:CGRectMake(10, self.frame.size.height - _normalHight-40-34, 34, 34) imageName:@"zhaochewei_dingwei_icon" tag:1];
    //3.创建放大按钮
    [self creatBtn:CGRectMake(self.frame.size.width - 40, self.frame.size.height - _normalHight - 70-40, 30, 35) imageName:@"dongtai_jiadian" tag:2];
    //4.创建缩小按钮
    [self creatBtn:CGRectMake(self.frame.size.width - 40, self.frame.size.height - _normalHight-35-40, 30, 35) imageName:@"dongtai_jiandian" tag:3];
}
...
-(void)reloadData:(FindMapData*)findMapData type:(int)type{
    ...
    switch (type) {
        ...
        case 6:
        {
            [self addCoord:findMapData type:6];
        }
            break;
        default:
            break;
...
//    if (findMaptData.coordArray.count == 0) {
//        return;
//    }
    int currentTemp = 0;
...
- (void)mapView:(BMKMapView *)mapView didSelectAnnotationView:(BMKAnnotationView *)view{
    if ([view.annotation isMemberOfClass:[ParkAnnotation class]]) {
...
-(void)reloadParkAnnotation:(NSString*)name{
    ...
    for (Park* coordinate in _findMapData.minorCoordArray) {
        ParkAnnotation* pointAnnotation = [[ParkAnnotation alloc]init];
        pointAnnotation.type = 3;
        if ([[NSString stringWithFormat:@"%i",temp] isEqualToString:name]) {
            pointAnnotation.type = 4;
    ...
    switch (routeAnnotation.type) {
        ...
        case 4:
        {
            ...
            if (view == nil) {
                ...
                view.image = [self getMinorAnnotationClickImageView];
                view.centerOffset = CGPointMake(0, -(view.frame.size.height * 0.5));
                view.canShowCallout = YES;
            }
            view.annotation = routeAnnotation;
        ...
        case 6:
        {
            view = [mapview dequeueReusableAnnotationViewWithIdentifier:@"carWash"];
            if (view == nil) {
                view = [[BMKAnnotationView alloc]initWithAnnotation:routeAnnotation reuseIdentifier:@"carWash"];
                view.image = [UIImage imageNamed:@"xiche_dian"];
                view.centerOffset = CGPointMake(0, -(view.frame.size.height * 0.5));
                view.canShowCallout = YES;
            }
            view.annotation = routeAnnotation;
        ...
        case 7:
        {
            ...
            if (view == nil) {
                view = [[BMKAnnotationView alloc]initWithAnnotation:routeAnnotation reuseIdentifier:@"oilCoordClick"];
                view.image = [UIImage imageNamed:@"xiche_dian_click"];
                view.centerOffset = CGPointMake(0, -(view.frame.size.height * 0.5));
                view.canShowCallout = YES;
            }
            view.annotation = routeAnnotation;
...
//    }
    UIImageView* lImageView = [[UIImageView alloc] initWithFrame:CGRectMake(27, 9, 30, 40)];
    switch (type) {
        case 1:
        {
//            lImageView.image = [UIImage imageNamed:@"zhaochewei_p2-"];
            lImageView.image = [UIImage imageNamed:@"chewei_location"];
...
//            lImageView.image = [UIImage imageNamed:@"zhaochewei_p_click2"];
            lImageView.image = [UIImage imageNamed:@"chewei_locationi_click"];
...
-(UIImage *)getMinorAnnotationImageView{
    ...
    imageView.image = [UIImage imageNamed:@"chewei_location1"];
    [markView addSubview:imageView];
    return [Utils imageFromView:markView];
    
}
#pragma mark - 获取水滴视图
-(UIImage *)getMinorAnnotationClickImageView{
    UIView* markView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kMARK_VIEW_WIDTH, kMARK_VIEW_HIGHT)];
    UIImageView* imageView = [[UIImageView alloc] initWithFrame:CGRectMake(27, 9, 20, 30)];
    imageView.image = [UIImage imageNamed:@"chewei_location_click1"];
    [markView addSubview:imageView];
    return [Utils imageFromView:markView];
