...
{
    //初始构造函数
    payUrl     = @"http://app.chcws.com:8080/XAI/app/pgy/appGainWechatOrder.do";
    if (debugInfo == nil){
...
- ( NSMutableDictionary *)sendPayWithPrice:(NSString *)thyPrice andOrderNum:(NSString*)thyOrderNum
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = @"车卫士充值";
    //订单金额,单位（分）
    float realPrice = [[[thyPrice componentsSeparatedByString:@"元"] firstObject] floatValue] * 100;
    NSString *order_price   = [NSString stringWithFormat:@"%d",(int)realPrice];
    ...
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: thyOrderNum           forKey:@"out_trade_no"];//商户订单号
