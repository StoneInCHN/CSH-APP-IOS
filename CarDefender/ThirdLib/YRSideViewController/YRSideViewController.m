...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(itemLeftClic:) name:@"itemLeftClic" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(itemRigthClic:) name:@"itemRigthClic" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(recoverNormal:) name:@"recoverNormal" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(slide:) name:@"slide" object:nil];
}
-(void)itemLeftClic:(NSNotification*)sender
{
...
-(void)recoverNormal:(NSNotification*)sender
{
    [self hideSideViewController:YES];
}
-(void)slide:(NSNotification*)sender
{
    NSString* slide = sender.object;
    if ([slide isEqualToString:@"0"]) {
        [self setNeedSwipeShowMenu:NO];
    }else{
...
- (void)showRightViewController:(BOOL)animated{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"changeRightBtn" object:@"1"];
    if (!_rightViewController) {
        return;
    }
...
- (void)hideSideViewController{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"changeRightBtn" object:@"0"];
    ...
    if (_panGestureRecognizer.state==UIGestureRecognizerStateBegan) {
        ...
        if(velocity.x>0){
            ...
            if (_currentView.frame.origin.x<=0 && _rightViewController && !_rightViewController.view.superview) {
                [self willShowRightViewController];
                [[NSNotificationCenter defaultCenter]postNotificationName:@"goRightRefreshMsg" object:nil];
            }
        }
        return;
    }
