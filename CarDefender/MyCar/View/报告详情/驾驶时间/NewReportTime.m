...
-(void)buildTopViewWithDic:(NSDictionary*)dicMsg
{
    ...
    if (_topBaseView==nil) {
        ...
        _topBaseView.layer.cornerRadius = 3;
    }
    ...
    if (_noData) {
        ...
        //创建绘图view
        if (_topView==nil) {
            _topView = [[ReportTopTime alloc]initWithFrame:CGRectMake(0, -2, _topBaseView.frame.size.width-_tableView.frame.size.width, _topBaseView.frame.size.height+2)];
...
-(void)buildDownViewWithDic:(NSDictionary*)dicMsg
{
    ...
    if (_noData) {
        ...
        contentString = [NSString stringWithFormat:@"其中最长连续驾驶%@分钟,击败了全国%@的车友",dicMsg[@"max"],dicMsg[@"percent"]];
        ...
        dic = @{@"title":@"您当日驾驶时长为",
                @"value":[self stringWihtTime:dicMsg[@"total"]],
                @"content":str,
                @"mark":markString};
    }
...
-(void)shareMsgEvent
{
    [self.delegate newRportTimeShareWithDic:@{@"content":_shareString,@"url":_dicMsg[@"shareUrl"],@"imgUrl":_dicMsg[@"shareIcon"]}];
}
#pragma mark - 刷新数据
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_selectPath!=nil) {
        if ([_selectPath isEqual:indexPath]) {
            cell.selectedBackgroundView.backgroundColor=kCOLOR(135, 135, 135);
        }
    }
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    currentCell.selectedBackgroundView.backgroundColor=kCOLOR(135, 135, 135);
