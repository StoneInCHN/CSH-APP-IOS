...
-(void)drawRect:(CGRect)rect
{
    ...
    if (self.pointArray.count==6) {
        ...
        CGRect historyMil=CGRectMake([self.pointArray[0] floatValue]/2, self.frame.size.height*0.3, weight+[self.pointArray[0] floatValue]/2, self.frame.size.height*0.7);
        CGContextSetFillColorWithColor(ctx, [UIColor grayColor].CGColor);
        // 填充一个矩形
        CGContextFillRect(ctx , historyMil);
        
        [self drawAllBtnFrame:historyMil];
    }
    
    
    CGFloat longHeight=[[NSString stringWithFormat:@"%@",_reportDic[@"max"]] floatValue];//路程最大值
    CGFloat shortHeight=[[NSString stringWithFormat:@"%@",_reportDic[@"min"]] floatValue];//路程最小值
    
    CGFloat btnLongHeight=self.frame.size.height*0.7*0.7;
    CGFloat btnShortHeight=25;
    CGFloat weightBtn;
    if (self.pointArray.count==6) {
    ...
    for (int i=(int)([arrayMsg count]-1); i<[arrayMsg count]; i--) {
        ...
        if (longHeight==shortHeight) {
            btnHeight=btnLongHeight;
        }else
            btnHeight=(btnLongHeight-btnShortHeight)*([dic[@"driverMile"] floatValue]-shortHeight)/(longHeight-shortHeight)+25;
        ...
        CGRect frame = CGRectMake(origX, self.frame.size.height-btnHeight, weightFloat, btnHeight);
        // 设置填充颜色
        UIColor*kColor;
...
-(void)drawAllBtnFrame:(CGRect)frameCurrent
{
    ...
    //定义4个点，绘制线段
    //计算字体长度确定标注长度
    NSString*startTime=[NSString stringWithFormat:@"%@km",_reportDic[@"mile"]];
...
-(void)drawTotalMieWithFrame:(CGRect)frameCurrent withDicMsg:(NSDictionary*)dicMsg
{
    ...
    UIFont  *font = [UIFont boldSystemFontOfSize:12];
    [startTime drawInRect:CGRectMake(wordsX, pointRightOrigin.y -ziSize.height, ziSize.width, ziSize.height) withFont:font];
    //写字
    [endTime drawInRect:CGRectMake(wordsX, pointRightOrigin.y , ziSize.width, ziSize.height) withFont:font];
    
    //左标示斜线
    //当前里程数
    NSString*currentMileage=dicMsg[@"driverMile"];
    currentMileage=[NSString stringWithFormat:@"%@km",currentMileage];
    CGSize milSize=[self takeTheSizeOfString:currentMileage withFont:[UIFont boldSystemFontOfSize:12.0]];
    CGContextSetLineWidth(ctx, 1);
    CGContextSetRGBStrokeColor(ctx, rColor/255, gColor/255, yColor/255, 1.0);
    
    CGFloat checkX=aRect1.origin.x+kRiderWH/2-10-milSize.width;
    if (checkX>0) {//左侧能够显示完全
