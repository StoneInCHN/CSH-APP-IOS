...
- (void)viewDidLoad {
    ...
    self.dicMsg = @{
                    ...
                    @"priceUnit":@"元/小时",
                    @"leftNum":@"658",
                    @"priceDesc":@"首两小时3元/小时，之后2元/小时，10元/12小时，20元/24小时，包月450"
                    };
    [self buildUI];
}
-(void)buildUI
{
    ...
    UILabel*nameLabel = [Utils labelWithFrame:CGRectMake(topXpoint, 15, kSizeOfScreen.width-topXpoint - kDistanceNomarl, 20) withTitle:self.dicMsg[@"name"] titleFontSize:kFontOfSize(16) textColor:kCOLOR(49, 52, 64) alignment:NSTextAlignmentLeft];
    ...
    UILabel*addressLabel = [Utils labelWithFrame:CGRectMake(topXpoint+imageAddress.frame.size.width+kDistanceNomarl-3, imageAddress.frame.origin.y, kSizeOfScreen.width-(topXpoint+imageAddress.frame.size.width+kDistanceNomarl+5) - kDistanceNomarl, 14) withTitle:self.dicMsg[@"addr"] titleFontSize:kFontOfSize(14) textColor:kCOLOR(121, 124, 128) alignment:NSTextAlignmentLeft];
    ...
    CGSize costSize = [Utils takeTheSizeOfString:self.dicMsg[@"priceDesc"] withFont:kFontOfSize(16) withWeight:kSizeOfScreen.width-(nubWeiImg.frame.size.width+kDistanceNomarl+3) - kDistanceNomarl];
    UILabel*costDetailLabel = [Utils labelWithFrame:CGRectMake(nubWeiImg.frame.size.width+kDistanceNomarl+3, costLabel.frame.size.height+costLabel.frame.origin.y+kDistanceNomarl, kSizeOfScreen.width-(nubWeiImg.frame.size.width+kDistanceNomarl+3) - kDistanceNomarl, costSize.height) withTitle:self.dicMsg[@"priceDesc"] titleFontSize:kFontOfSize(16) textColor:kCOLOR(51, 51, 51) alignment:NSTextAlignmentLeft];
    ...
    [leftBtn setTitleColor:kCOLOR(233, 233, 233) forState:UIControlStateNormal];
